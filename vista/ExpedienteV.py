...
def crear_ventana(voz_controlador, usuario_id_actual=1):
    root = tk.Tk()
    ...
    def btn_img(master, filename, text, cmd):
        ...
        b = tk.Button(master, image=img, text=text, compound="left", font=("Arial",10), command=cmd)
        b.image = img
        return b
    ...
    def funcion_guardar():
        ...
        try:
            ...
            print(f"[GUARDADO] ID={id} →", flat)
            messagebox.showinfo("Éxito", "Expediente guardado correctamente.")
        except Exception as e:
            print("Error al guardar el expediente:", e)
            traceback.print_exc()
            messagebox.showerror("Error", f"Error al guardar el expediente:\n{e}")
        # Navegación por tabs
    ...
    btn_save = btn_img(frame_bot, "guardar-el-archivo.png", "Guardar", funcion_guardar)
    btn_exit = btn_img(frame_bot, "cerrar-sesion.png", "Salir", salir_y_regresar_menu)

    for w in (btn_grab, btn_stop, btn_save): w.pack(side="left", padx=5)
